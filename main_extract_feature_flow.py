...
def train_extract(train_loader, model, flow, criterion, optimizer, scaler, epoch, lr_schedule, args):
	...
	with torch.no_grad():
		for data_iter, (images, images_flow, target) in enumerate(train_loader):				
			#print(images.shape, images_flow.shape)
			images = images.cuda(args.gpu, non_blocking=True).view(-1,3,224,224)
			target = target.cuda(args.gpu, non_blocking=True)
			images_flow = images_flow.cuda(args.gpu, non_blocking=True).view(-1,3,224,224)
			#print(images.shape, images_flow.shape)

			# compute output
			#with amp.autocast(enabled=not args.disable_amp):
			# Temporarily disable cuDNN for flow computation to avoid non-contiguous tensor issues
			with torch.backends.cudnn.flags(enabled=False):
				flow_out = flow(images.contiguous(), images_flow.contiguous())
			flow_img = flow_to_image(flow_out[-1]).view(-1,3,16,224,224)/255.0
			output = model(flow_img)
		...
		for i, (images, images_flow, target) in enumerate(val_loader):
			...
			if isinstance(images, list):
				...
				for crop, crop_flow in zip(images, images_flow):
					...
					# Temporarily disable cuDNN for flow computation to avoid non-contiguous tensor issues
					with torch.backends.cudnn.flags(enabled=False):
						flow_out = flow(crop.view(-1,3,224,224).contiguous(), crop_flow.view(-1,3,224,224).contiguous())
					flow_img = flow_to_image(flow_out[-1]).view(-1,3,16,224,224)/255.0
					logit = model(flow_img)
