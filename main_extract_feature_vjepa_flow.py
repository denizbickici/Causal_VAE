...
def build_transforms(args):
	...
	if args.model_type == 'mvit_temporal' or args.model_type in VJEPA2_MODEL_SPECS:
...
def extract_split(loader, model, args, subset: str):
...
def main(args):
	...
	if args.model_type in VJEPA2_MODEL_SPECS:
		for key in ("encoder.patch_embed.proj.weight", "patch_embed.proj.weight"):
			if key in new_state_dict:
				patch_key = key
				break
		if patch_key is not None and new_state_dict[patch_key].shape[1] == 2:
	...
	if args.model_type in VJEPA2_MODEL_SPECS:
		if patch_key is None or new_state_dict[patch_key].shape[1] == 3:
			adapt_vjepa_for_flow(model)
	...
	# Extract features
	extract_split(train_loader, model, args, subset='train')
	extract_split(val_loader, model, args, subset='test')
