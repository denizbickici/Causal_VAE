...
#SBATCH --job-name="extract_vjepa_probe"
...
# Launcher for extracting V-JEPA2 probe features.
# Usage:
...
#
# MODEL_TYPE options (from main_extract_feature_vjepa_probe.py):
...
if [ "$MULTI_TASK" = "1" ]; then
    ...
    PRETRAIN_MODEL="${PRETRAIN_MODEL:-/scratch/users/bickici/data/EK100/model_checkpoints/${MODEL_TYPE}_${DATASET}_multitask/checkpoint_best.pt}"
else
    OUTPUT_DIR="/scratch/users/bickici/data/EK100/vjepa_features/${MODEL_TYPE}_${DATASET}_${TASK_TYPE}"
    if [ "$TASK_TYPE" = "verb" ]; then
        NUM_CLASSES_FLAG="--num-classes 97"
    elif [ "$TASK_TYPE" = "noun" ]; then
        NUM_CLASSES_FLAG="--num-classes 300"
    elif [ "$TASK_TYPE" = "action" ]; then
        NUM_CLASSES_FLAG="--num-classes 3806"
    fi
fi
...
CMD="python3 main_extract_feature_vjepa_probe.py \
    --dataset $DATASET \
    --task-type $TASK_TYPE \
